...
async function main(){const r=await fetch('data/latest.json?'+Date.now());const d=await r.json();
updated.textContent='最近更新：'+d.updated_at+'（土耳其时间）';k1.textContent=d.kpis.top_category;k1n.textContent=d.kpis.top_category_note;k2.textContent=d.kpis.cross_platform;k3.textContent=d.kpis.data_status;
summaryText.innerHTML=d.summary.map(x=>`<p>${esc(x)}</p>`).join('');
function rows(arr,id){document.getElementById(id).innerHTML=arr.map((x,i)=>`<tr data-s="${esc([x.platform,x.category,x.title,x.source].join(' ').toLowerCase())}"><td><b>${i+1}</b></td><td><span class="badge">${esc(x.platform)}</span></td><td>${esc(x.category)}</td><td><b>${esc(x.title)}</b><br><small>${esc(x.hook)}</small></td><td>${esc(x.source)}</td><td>${esc(x.signal)}</td><td>${esc(x.why)}</td><td>${x.url?`<a class="link" target="_blank" rel="noopener" href="${esc(x.url)}">打开</a>`:'待核验'}</td></tr>`).join('')}
...
const cats={};[...d.global_top10,...d.china_top10].forEach(x=>cats[x.category]=(cats[x.category]||0)+1);
...
const plats={};[...d.global_top10,...d.china_top10].forEach(x=>plats[x.platform]=(plats[x.platform]||0)+1);
